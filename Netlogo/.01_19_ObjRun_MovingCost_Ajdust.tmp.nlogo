...
to Ori_change_color
  if breed = normals[
    if Government_strategy = "One-time-Subsidy" [
      if (Total_market_value + 00000) <= Future_loss [
        if Ori_moved? = False [
          set Ori_moved? True
          set Ori_year TIME
        ]
  ...
  if breed = poors[
    if (Total_market_value + 400000) <= Future_loss [
       if Ori_moved? = False [
          set Ori_moved? True
          set Ori_year TIME
        ]
...
to Change_color ;; NEW COMMENT: Moving cost is fixed
  ...
  if breed = normals[
    if Government_strategy = "One-time-Subsidy" [
      if (Total_market_value + 400000) - Subsidy - Future_loss <= threshold  [
        if moved? = False [
...
<experiments>
  <experiment name="best_Subsidy" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="false">
    ...
    <steppedValueSet variable="Subsidy" first="0" step="20000" last="1000000"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
