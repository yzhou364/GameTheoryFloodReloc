;; NEW COMMENT: Update 01/20/2020
;; Don't forget to adjust the period (in the Interface) to 80 years
;; Subsidy now becomes individual subsidy which is specific to each house based on their house price with subsidy percentage as a parameter
;; This is the Objective Run version of code with no for the neighborhood effect
;; Additional moving cost has been adjusted to be a parameter
...
;  Update_coefficient_SUBSIDY_PV ;; No longer needed for the subsidy by percentage case
...
;  ]
  ...
  Update_coefficient_TOTAL  ;; update values and coefficient in each tick
;  Update_coefficient_SUBSIDY_PV ;; update subsidy pv value ;; No longer needed for the subsidy by percentage case
...
to House_property
  foreach gis:feature-list-of GEODATASET [ vector-feature ->
    ...
    if not empty? loc[
      ...
      Setting_agents
      set Indiv_Subsidy (Total_market_value * Subsidy_percentage) ;; Total_market_value is varied by household, Subsidy_percentage is an adjustable parameter
      ]
    ]
  ]
end
...
;to Update_coefficient_SUBSIDY_PV    ;; function to update one-time subsidy ;; Not needed when we consider individual subsidy
...
to Ori_change_color
  if breed = normals[
    if Government_strategy = "One-time-Subsidy" [
      if (Total_market_value + MOVING_COST_ADDITION) <= Future_loss [
        if Ori_moved? = False [
          set Ori_moved? True
          set Ori_year TIME
        ]
...
to Change_color ;; NEW COMMENT: Moving cost is fixed
  ...
  if breed = normals[
    if Government_strategy = "One-time-Subsidy" [
      if (Total_market_value + MOVING_COST_ADDITION) - Future_loss <= threshold[
        if moved? = False[
          set moved? True
          set Mot_year TIME
        if Flood_type = "Multiple"[
      ...
      if (Total_market_value + MOVING_COST_ADDITION) - (Indiv_Subsidy * ( 1 + Government_dis) ^ TIME) - Future_loss <= threshold  [
        if moved? = False [
        ...
        if Flood_type = "Multiple"[
        set OBJECTIVE OBJECTIVE + ( ( (item 2 PAST_LOSS ) * (Structure_Value * Damage_pct_10Y + Sq.ft. / 2500 * Cost_to_personal_property_10Y) ) + ( (item 5 PAST_LOSS) * (Structure_Value * Damage_pct_100Y + Sq.ft. / 2500 * Cost_to_personal_property_100Y) ) )
        set OBJECTIVE OBJECTIVE +  (Indiv_Subsidy * ( 1 + Government_dis) ^ TIME)
          ]
        if Flood_type = "100_year" [
        set OBJECTIVE OBJECTIVE +  ( (item 2 PAST_LOSS) * (Structure_Value * Damage_pct_100Y + Sq.ft. / 2500 * Cost_to_personal_property_100Y) )
        set OBJECTIVE OBJECTIVE +  (Indiv_Subsidy * ( 1 + Government_dis) ^ TIME)
...
@#$#@#$#@
<experiments>
  <experiment name="best_SubsidyPerc" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>OBJECTIVE</metric>
    <steppedValueSet variable="Subsidy_percentage" first="0.02" step="1.0E-4" last="0.07"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
