...
to House_property
  foreach gis:feature-list-of GEODATASET [ vector-feature ->
    ...
    if not empty? loc[
      ...
      set xcor item 0 loc
      set ycor item 1 loc
      set Elevation gis:property-value vector-feature "TOTALMARKE"
      set Stories gis:property-value vector-feature "TOTALMARKE"
...
to Setting_agents
  ifelse Total_market_value >= 3000 [
    set breed normals
    set shape "triangle"
    set color blue
  ]
...
to Change_color
  ...
  [
    ifelse Government_strategy = "One-time-Subsidy" [
      if Total_market_value - ( item 0 SUBSIDY_PV)- future_loss <= threshold [
        set color red  ;; change color to red if moved
        set moved? True ;; resident has moved
      ]
    ]
    [
      if Total_market_value - future_benefit - future_loss <= threshold [
        set color red  ;; change color to red if moved
        set moved? True ;; resident has moved
      ]
    ]
