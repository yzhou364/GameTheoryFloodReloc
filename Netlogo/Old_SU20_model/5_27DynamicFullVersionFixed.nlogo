...
[
  ...
  ;; Miscellaneous list
  TIME GEODATASET CLASS SUM_SUBSIDY MOVE_REACHED? TIME_VAR NEW_SUBSIDY ORDER_SUB PERCENTILE REM
]
...
to Update_coefficient_SUBSIDY_PV_Dynamic
  ...
  [
    ...
    [
        if (TIME >= Renewal_Year) and (TIME mod Renewal_Year = 0)
        ;[ set Subsidy NEW_SUBSIDY ]
        [set Subsidy New_Target_Moved_Rate * SUM_SUBSIDY]

        ;; [ifelse subsidy >=0 [set Subsidy New_Target_Moved_Rate * SUM_SUBSIDY][set Subsidy 0]]
        Update_coefficient_SUBSIDY_PV
    ]
...
;  set new_Subsidy REM * ((position (ceiling REM) ORDER_SUB) - (position (floor REM) ORDER_SUB)) + position (floor REM) ORDER_SUB
;end
...
;    Dynamic_Subsidy_Trigger = "Year"
;    [
;       (ifelse TIME mod Renewal_Year = 0
;        [ set TIME_VAR 0
;          set subsidy New_Target_Moved_Rate * sum(S_prime) ]
;        ; else
;        [ set TIME_VAR remainder TIME Renewal_Year ])
;
;        Update_coefficient_SUBSIDY_PV
;      ]
;
;;        ifelse TIME mod Renewal_Year = 0
;;        [
;;          set subsidy New_Target_Moved_Rate * sum(S_prime) ;;; sum(S_prime) NEEDED to be fixed
;;          set TIME_VAR remainder TIME Renewal_Year ;; New SUBSIDY_PV for that interval
;;          Update_coefficient_SUBSIDY_PV
;;        ]
;;        [
;;          set TIME_VAR remainder TIME Renewal_Year ;; New SUBSIDY_PV for that interval
...
;
;
;;        if TIME mod Renewal_Year = 0
;;        [
;;          set subsidy New_Target_Moved_Rate * sum(S_prime) ;;; sum(S_prime) NEEDED to be fixed
;;          let SUBSIDY_TIME 0 ;; New SUBSIDY_PV for that interval
;;          set TIME_VAR SUBSIDY_TIME
;;          loop
;;          [
;;            if SUBSIDY_TIME >= remainder TIME Renewal_Year [stop]
;;            Update_coefficient_SUBSIDY_PV
;;            set SUBSIDY_TIME SUBSIDY_TIME + 1
;;          ]
;;        ]
;;      ]
;    Dynamic_Subsidy_Trigger = "Percentage_Past_Moved"
...
;    set SUBSIDY_PV replace-item 0 SUBSIDY_PV ( Subsidy / ( 1 + Normal_dis ) ^ TIME_VAR )
;    set SUBSIDY_PV replace-item 1 SUBSIDY_PV ( Subsidy / ( 1 + Poor_dis ) ^ TIME_VAR )
;    set SUBSIDY_PV replace-item 2 SUBSIDY_PV ( Subsidy / ( 1 + Government_dis) ^ TIME_VAR )

end

;to Dynamic_Subsidy_Calculation_Renewal_Year
;
;  ;set subsidy New_Target_Moved_Rate * sum(S_prime) ; s_prime not move ;; New Subsidy
;  let SUBSIDY_TIME 0 ;; New SUBSIDY_PV for that interval
;  loop
;  [
;    if SUBSIDY_TIME >= remainder TIME Renewal_Year [stop]
;    set SUBSIDY_PV replace-item 0 SUBSIDY_PV ( Subsidy / ( 1 + Normal_dis ) ^ SUBSIDY_TIME )
;    set SUBSIDY_PV replace-item 1 SUBSIDY_PV ( Subsidy / ( 1 + Poor_dis ) ^ SUBSIDY_TIME )
;    set SUBSIDY_PV replace-item 2 SUBSIDY_PV ( Subsidy / ( 1 + Government_dis) ^ SUBSIDY_TIME )
;    set SUBSIDY_TIME SUBSIDY_TIME + 1
;  ]
;end

;to Opportunity_Cost_Calculation
;  ifelse Dynamic_Subsidy?
;  [
;    set OPP_COST replace-item 0 OPP_COST sum(item 0 SUBSIDY_PV)
;    set OPP_COST replace-item 1 OPP_COST sum(item 1 SUBSIDY_PV)
;    set OPP_COST replace-item 2 OPP_COST sum(item 2 SUBSIDY_PV)
;  ]
;
;  [
;    set OPP_COST replace-item 0 OPP_COST 0
;    set OPP_COST replace-item 1 OPP_COST 0
;    set OPP_COST replace-item 2 OPP_COST 0
;  ]
;
;end
...
to Ori_change_color ;; to determine moved households without an influence of the subsidy ;; ONLY One-time-Subsidy CASE
  ...
  [
    ...
    [
      ...
      ;if Total_market_value  *  Moving_Cost_Multiplier - (item 0 OPP_COST) - Future_loss <= threshold
      [
        if Ori_moved? = False
        [
          set Ori_moved? True
  ...
  [
    ...
    [
      if Total_market_value  *  Moving_Cost_Multiplier - Future_loss <= threshold
      ;if Total_market_value  *  Moving_Cost_Multiplier - (item 1 OPP_COST) - Future_loss <= threshold
      [
        if Ori_moved? = False
        [
          set Ori_moved? True
...
to Change_color ;; if the moving cost minus the pv of subsidy is less than the future loss, then residents will move ;; ONLY One-time-Subsidy CASE ;;;INDEX MIGHT BE WRONG, FIXED it already
  ...
  [
    ...
    [
      ...
      ;ifelse Total_market_value * Moving_Cost_Multiplier - Subsidy - (item 0 OPP_COST) - Future_loss <= threshold
      [
        if Moved? = False
        [
          if Total_market_value * Moving_Cost_Multiplier - Future_loss < S_prime
          ;if Total_market_value * Moving_Cost_Multiplier - (item 0 OPP_COST) - Future_loss < S_prime
          [
            set S_prime Total_market_value * Moving_Cost_Multiplier - Future_loss
            ;set S_prime Total_market_value * Moving_Cost_Multiplier - (item 0 OPP_COST) - Future_loss
          ]
      ...
      ;; else
      ;;[if Moved? = False [ set SUM_SUBSIDY lput (Total_market_value * Moving_Cost_Multiplier - (item 0 OPP_COST) - Future_loss) SUM_SUBSIDY] ]
      ;[if Moved? = False [ set SUM_SUBSIDY insert-item 0 SUM_SUBSIDY (Total_market_value * Moving_Cost_Multiplier - (item 0 OPP_COST) - Future_loss) SUM_SUBSIDY] ]
      ;[if Moved? = False [ set SUM_SUBSIDY SUM_SUBSIDY + (Total_market_value * Moving_Cost_Multiplier - (item 0 OPP_COST) - Future_loss)]
      [if Moved? = False [ set SUM_SUBSIDY SUM_SUBSIDY + (Total_market_value * Moving_Cost_Multiplier - Future_loss)]
      ]
    ]
  ]
  ...
  [
    ...
    [
      ifelse Total_market_value * Moving_Cost_Multiplier - Subsidy - Future_loss <= threshold
      ;ifelse Total_market_value * Moving_Cost_Multiplier - Subsidy - (item 1 OPP_COST) - Future_loss <= threshold
      [
        if moved? = False
        [
         if Total_market_value * Moving_Cost_Multiplier - Future_loss < S_prime
         ;if Total_market_value * Moving_Cost_Multiplier - (item 1 OPP_COST) - Future_loss < S_prime
          [
            set S_prime Total_market_value * Moving_Cost_Multiplier - Future_loss
            ;set S_prime Total_market_value * Moving_Cost_Multiplier - (item 1 OPP_COST) - Future_loss
          ]
      ...
      ;;else
      [if Moved? = False
          ;[ set SUM_SUBSIDY lput (Total_market_value * Moving_Cost_Multiplier - (item 1 OPP_COST) - Future_loss) SUM_SUBSIDY] ]
          ;[ set SUM_SUBSIDY SUM_SUBSIDY + (Total_market_value * Moving_Cost_Multiplier - (item 1 OPP_COST) - Future_loss) ] ]
          [ set SUM_SUBSIDY SUM_SUBSIDY + (Total_market_value * Moving_Cost_Multiplier - Future_loss) ] ]
    ]
  ]
end
