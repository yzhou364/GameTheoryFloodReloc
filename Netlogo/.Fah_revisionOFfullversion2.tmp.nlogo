...
to Influence  ;;determine relocated house influenced by their neighbors, must go along with the running years
  ...
  [
    ...
    if Moved? = false
    [
      set nearhouse min-n-of 10 normals [distance myself] ;set color white ;; neigbors = n - 1
...
to Update_coefficient_TOTAL ;; function to calculate the coefficient of the future loss of the residents
...
to Ori_change_color ;; to determine moved households without an influence of the subsidy
  if breed = normals
  [
...
to Change_color ;; if the moving cost minus the pv of subsidy is less than the future loss, then residents will move
  if breed = normals
  [
    if Government_strategy = "One-time-Subsidy"
    [
