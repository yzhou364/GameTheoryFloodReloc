...
[
  ...
  TOTAL
  ; Miscellaneous
  MOTIVATED move_counter notmove_counter normalmove_counter poormove_counter normalinf_counter poorinf_counter
...
[
  ...
  ;; Moving
  Mot_year Ori_year Ori_moved? nearhouse move normal_inf? poor_inf? S_prime Moved? cnear nbpast_move ;normalmove_counter poormove_counter
]
...
to Go
  ...
  set move_counter count (turtles with [Moved? = true])
  set notmove_counter count (turtles with [Moved? = false])
  ask turtles
  [ (if breed = normals [
    set normalmove_counter count (turtles with [Moved? = true])
    set normalinf_counter count (turtles with [Mot_year != Ori_year])
    ])
    (if breed = poors [
    set poormove_counter count (turtles with [Moved? = true])
    set poorinf_counter count (turtles with [Mot_year != Ori_year])
      ])
  ]
  tick
...
to Influence  ;;determine relocated house influenced by their neighbors, must go along with the running years
              ;;self-moved before relying on neighbor's influences

;  if breed = normals
;  [
;    if Moved? = false
;    [
;      set nearhouse min-n-of 4 normals [distance myself] ;set color white ;; neigbors = n - 1
...
;    if Moved? = false
;    [
;      set nearhouse min-n-of 4 poors [distance myself]; set color yellow
;      set move count (nearhouse with [Moved? = true]); and ori_moved? = true])
;      if (nbpast_move < move)
;      [
;        if random-float 100 <= Neighbor_Influence_Probability
...
;  ]
  ...
  [
    ...
    if Moved? = false
    [
      set nearhouse min-n-of 2 other normals [distance myself] ;set color white ;; neigbors = n
      set move count (nearhouse with [Moved? = true]); and ori_moved? = true])
      if (nbpast_move < move)      ; to compare the difference of neighbor's influence
      [
        if random-float 100 < Neighbor_Influence_Probability
  ...
  [
    ...
    if Moved? = false
    [
      set nearhouse min-n-of 2 other poors [distance myself]; set color yellow
      set move count (nearhouse with [Moved? = true]); and ori_moved? = true])
      if (nbpast_move < move)
      [
        if random-float 100 < Neighbor_Influence_Probability
