...
to Change_color
  ...
  if breed = normals[
    if Government_strategy = "One-time-Subsidy" [
      if Total_market_value  * Moving_Cost_Multiplier - Subsidy * Total_market_value - Future_loss <= threshold [
        if moved? = False [
        ...
        set OBJECTIVE OBJECTIVE +  (item 2 SUBSIDY_PV * Total_market_value)
          ]
        if Flood_type = "100_year" [
...
@#$#@#$#@
<experiments>
  <experiment name="best_percentage" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>OBJECTIVE</metric>
    <steppedValueSet variable="Subsidy" first="0.01" step="0.01" last="1"/>
    <steppedValueSet variable="Moving_Cost_Multiplier" first="1" step="1" last="20"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
