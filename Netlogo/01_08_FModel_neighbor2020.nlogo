;; This version has been adjusted for neighborhood effects
;; probability data for both 10- and 100-year flood have been adjusted to 80 years
;; ABM modeling year starts at 2020 (adjust the probability files)
;; the latest version as of 01/08/21

;; Interface adjustment: set period to 80 years, otherwise index error may occur
;; Code adjustment: set TIME > 80 in Go function
...
to Setup
  ...
  random-seed 10000000      ;; set a fixed random seed to maintain the same result after refresh
  Initialize_list
  ...
  set SUBSIDY_MULTIPLIER Subsidy_Percentage
  reset-ticks
end

to Go
  ;; stop condition is that every agent has moved
;  if TIME > 100 [ stop ] ;; from year 2000-2100
  if TIME > 80 [ stop ] ;; from year 2020-2100
...
;  ]
  ...
  [
    ...
    Ori_change_color ;; change color if agents originally moved in this year (willing to move without incentive)
    Change_color     ;; change color if agents moved in this year (move because of the incentive)
...
;        set nbpast_move move ;; original version: enable this line of code
      ]
    ]
  ]
  if breed = poors
...
to Update_coefficient_TOTAL ;;; I think the INDEX MIGHT BE WRONG here, so I've adjusted the function. This is Fah's version
                            ;; function to calculate the coefficient of the future loss of the residents
                            ;; coefficient of expected future loss
  let iter TIME + 1 ;; NOT using this case, please skips this section and refers to the non-hyperbolic case
  if Hyperbolic?
  [
    if Flood_type = "10_year"
    [
      loop
      [
        if iter > period [stop] ;; stop condition is fixed to avoid the range out of period
...
to Update_values ;; Function to update the future loss value for each home
  ...
  ifelse Fixed_Relocation_Cost?
  [
    if breed = normals
    [
      if Flood_type = "10_year"
      [
        set Future_loss (item 0 TOTAL) * (Total_market_value + 300000)
      ]
      if Flood_type = "100_year"
      [
        set Future_loss (item 0 TOTAL) * (Total_market_value + 300000)
      ...
      if Flood_type = "Multiple"
      [
        set Future_loss (((item 3 TOTAL) * (Total_market_value + 300000) + ((item 0 TOTAL) *  (Total_market_value + 300000))))
      ]
    ]
    if breed = poors
    [
      if Flood_type = "10_year"
      [
        set Future_loss (item 1 TOTAL) * (Total_market_value + 300000)
      ]
      if Flood_type = "100_year"
      [
        set Future_loss (item 1 TOTAL) * (Total_market_value + 300000)
      ...
      if Flood_type = "Multiple"
      [
        set Future_loss (((item 4 TOTAL) * (Total_market_value + 300000) + ((item 1 TOTAL) *  (Total_market_value + 300000))))
      ]
    ]
  ]
  ;;else condition
...
to Damage_percentage_onestory_nobasement ;our current case
  ...
  Inundation_10Y <= 6 [set Damage_pct_10Y 0.1835] ;;Testing. Interpolation of 13.4% and 23.3%
  Inundation_10Y <= 12 [set Damage_pct_10Y 0.233]
  Inundation_10Y <= 24 [set Damage_pct_10Y 0.321]
  Inundation_10Y <= 36 [set Damage_pct_10Y 0.401]
  Inundation_10Y <= 48 [set Damage_pct_10Y 0.471]
  Inundation_10Y <= 60 [set Damage_pct_10Y 0.532]
  Inundation_10Y <= 72 [set Damage_pct_10Y 0.586]
  Inundation_10Y <= 84 [set Damage_pct_10Y 0.632]
  Inundation_10Y <= 96 [set Damage_pct_10Y 0.672]
  Inundation_10Y <= 108 [set Damage_pct_10Y 0.705]
  Inundation_10Y <= 120 [set Damage_pct_10Y 0.732]
  Inundation_10Y > 120 [set Damage_pct_10Y 0.754]) ;Inundation > 120 consider the same high damage to the house
  ...
  Inundation_100Y <= 48 [set Damage_pct_100Y 0.471]
  Inundation_100Y <= 60 [set Damage_pct_100Y 0.532]
  Inundation_100Y <= 72 [set Damage_pct_100Y 0.586]
  Inundation_100Y <= 84 [set Damage_pct_100Y 0.632]
  Inundation_100Y <= 96 [set Damage_pct_100Y 0.672]
  Inundation_100Y <= 108 [set Damage_pct_100Y 0.705]
  Inundation_100Y <= 120 [set Damage_pct_100Y 0.732]
  Inundation_100Y > 120 [set Damage_pct_100Y 0.754])  ;Inundation > 120 consider the same high damage to the house
end
...
to Damage_percentage_morethanonestory_nobasement
  ...
  (ifelse
  Inundation_10Y <= 0 [set Damage_pct_10Y 0]
  Inundation_10Y <= 12 [set Damage_pct_10Y 0.152]
  Inundation_10Y <= 24 [set Damage_pct_10Y 0.209]
  Inundation_10Y <= 36 [set Damage_pct_10Y 0.263]
  Inundation_10Y <= 48 [set Damage_pct_10Y 0.314]
  Inundation_10Y <= 60 [set Damage_pct_10Y 0.362]
  Inundation_10Y <= 72 [set Damage_pct_10Y 0.407]
  Inundation_10Y <= 84 [set Damage_pct_10Y 0.449]
  Inundation_10Y <= 96 [set Damage_pct_10Y 0.488]
  Inundation_10Y <= 108 [set Damage_pct_10Y 0.524]
  Inundation_10Y <= 120 [set Damage_pct_10Y 0.557]
  Inundation_10Y > 120 [set Damage_pct_10Y 0.587]) ;Inundation > 120 consider the same high damage to the house

  set Damage_pct_100Y 0
  (ifelse
  Inundation_100Y <= 0 [set Damage_pct_100Y 0]
  Inundation_100Y <= 12 [set Damage_pct_100Y 0.152]
  Inundation_100Y <= 24 [set Damage_pct_100Y 0.209]
  Inundation_100Y <= 36 [set Damage_pct_100Y 0.263]
  Inundation_100Y <= 48 [set Damage_pct_100Y 0.314]
  Inundation_100Y <= 60 [set Damage_pct_100Y 0.362]
  Inundation_100Y <= 72 [set Damage_pct_100Y 0.407]
  Inundation_100Y <= 84 [set Damage_pct_100Y 0.449]
  Inundation_100Y <= 96 [set Damage_pct_100Y 0.488]
  Inundation_100Y <= 108 [set Damage_pct_100Y 0.524]
